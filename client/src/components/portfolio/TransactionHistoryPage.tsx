          ...
          <tbody>
            {sorted.map((tx) => (
              <tr key={tx.id} style={{ borderTop: `1px solid ${theme.border}` }}>
                <td style={{ padding: '12px 16px', textAlign: 'right', color: theme.text.secondary }}>{tx.date}</td>
                ...
                <td style={{ padding: '12px 16px', textAlign: 'right', color: theme.text.primary }}>${fmt(tx.priceAtDate)}</td>
                <td style={{ padding: '12px 16px', textAlign: 'right', color: theme.text.primary }}>{tx.shares.toFixed(4)}</td>
                <td style={{ padding: '12px 16px', textAlign: 'right', color: theme.text.primary }}>${fmt(tx.amount)}</td>
                <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                  <button
