...
const SECTIONS: Section[] = [
  {
    title: '홈',
    intro: '앱을 처음 열면 보이는 기본 화면입니다. 시장 현황과 국제 뉴스 서브탭으로 구성되어 있습니다.',
    items: [
      { label: '시장 현황', desc: '나스닥·S&P500·다우·러셀2000 선물, USD/KRW 환율, 달러 인덱스, 2년·10년 국채, VIX, 금·은·WTI 원유 선물, 코스피·코스닥, 비트코인·이더리움 등 16개 주요 지표의 실시간 시세를 표시합니다. 카드를 클릭하면 해당 종목 정보 화면으로 이동합니다. 헤더의 ? 버튼을 누르면 각 지표(선물·달러 인덱스·국채·VIX)에 대한 설명을 볼 수 있습니다.' },
      { label: '풋/콜 비율', desc: 'SPY·QQQ의 전체 만기 기준 풋콜 비율을 표시합니다. 비율이 1 초과면 약세(풋 우세), 1 미만이면 강세(콜 우세) 신호로 해석합니다. ? 버튼으로 상세 설명을 볼 수 있습니다.', note: '풋콜 비율은 단독 지표보다 다른 지표와 함께 참고하는 것이 좋습니다.' },
      { label: '국제 뉴스', desc: 'Reuters, CNBC, AP News, Investing.com, Yahoo Finance, Google News 등 주요 뉴스를 최신순으로 모아 보여줍니다. 스크롤을 내리면 자동으로 추가 뉴스를 불러오는 무한 스크롤 방식입니다.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '장 구분 뱃지', desc: '본장(초록), 프리장(주황), 애프터장(보라), 장마감(회색), 휴장(회색)으로 현재 어느 장이 열려 있는지 표시합니다. 프리장이나 애프터장 데이터가 있으면 해당 가격과 등락도 함께 보여줍니다. 주말·공휴일에는 휴장으로 표시됩니다.', note: '미국 주식은 본장 외에도 프리마켓(장 시작 전)과 애프터마켓(장 마감 후)에 거래됩니다.' },
      ...
      { label: '즐겨찾기 (★)', desc: '★ 버튼을 누르면 즐겨찾기에 추가되고 금색으로 바뀝니다. 다시 누르면 해제됩니다. 추가된 종목은 즐겨찾기 탭에서 한눈에 볼 수 있습니다. 시장 현황(홈 탭)에 있는 16개 지표 종목에는 ★ 버튼이 표시되지 않습니다.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '인터벌', desc: '1분·1시간·1일·1주·1달·12달 단위로 인터벌을 변경할 수 있습니다.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '계좌 잔액 제한', desc: '매수 금액이 계좌 잔액을 초과하면 매수가 불가합니다. 잔액 부족 시 추가로 필요한 금액(매수 금액 − 잔액)을 화면에 표시해줍니다.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '삭제', desc: '잘못 입력한 거래를 삭제할 수 있습니다. 삭제 버튼을 누르면 확인 모달이 표시되며, 확인 후 삭제됩니다. 삭제 시 해당 금액만큼 계좌 잔액이 자동으로 복구됩니다.' },
    ],
  },
  {
    title: '즐겨찾기',
    intro: '관심 있는 종목을 그룹별로 저장해두고 현재 시세를 빠르게 확인할 수 있습니다.',
    items: [
      { label: '추가 / 해제', desc: '종목 정보 화면의 ★ 버튼으로 추가하거나 해제합니다. 즐겨찾기 목록에서 ★ 버튼을 눌러도 해제됩니다. 처음 추가되는 종목은 첫 번째 그룹에 자동으로 들어갑니다.' },
      { label: '그룹 관리', desc: '상단의 "+ 그룹 추가" 버튼으로 새 그룹을 만들 수 있습니다. 각 그룹 오른쪽의 "이름 변경" 버튼으로 그룹명을 수정하고, "삭제" 버튼으로 그룹을 제거합니다. 그룹을 삭제하면 해당 그룹의 종목은 첫 번째 그룹으로 이동합니다. 그룹이 하나뿐일 때는 삭제 버튼이 표시되지 않습니다.' },
      { label: '그룹 간 이동', desc: '그룹이 2개 이상일 때 각 종목 행에 드롭다운이 표시됩니다. 드롭다운으로 해당 종목을 다른 그룹으로 이동할 수 있습니다.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '데이터 저장 방식', desc: '거래내역·계좌 잔액은 브라우저 세션 스토리지에, 즐겨찾기·테마 설정은 로컬 스토리지에 저장됩니다. 서버에 계정이 없으므로 브라우저 데이터를 삭제하거나 탭을 닫으면(세션 스토리지) 또는 다른 기기에서 접속하면 데이터가 초기화될 수 있습니다.' },
